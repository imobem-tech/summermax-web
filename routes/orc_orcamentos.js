...
router.post('/', autorizar('OPERADOR', 'ADMIN_EMPRESA', 'MASTER_GRUPO', 'SUPER_ADMIN'), async (req, res) => {
  ...
  try {
    ...
    const {
      id_embarcacao,
      id_fornecedor,
      titulo,           // ✅ Aceita "titulo" (novo)
      descricao,        // ⚠️ Aceita "descricao" (legado)
      observacao,
      itens,
      taxa_servico_perc,
      forma_pagamento,  // Opcional - será definido na etapa de cobrança
      qtd_parcelas,     // Opcional
      dias_vencimento   // Opcional
    } = req.body;

    // Usar título se vier, senão usar descrição (compatibilidade)
    const tituloOrcamento = titulo || descricao;

    // Validações
    if (!id_embarcacao || !tituloOrcamento || !itens || itens.length === 0) {
      await client.query('ROLLBACK');
      return res.status(400).json({ erro: 'Dados obrigatórios faltando (embarcação, título, itens)' });
    }
    ...
    `, [
      id_grupo, id_empresa, numero, id_embarcacao, id_fornecedor,
      tituloOrcamento, observacao,
      valor_servicos, taxa_perc, taxa_servico_valor, valor_total,
      forma_pagamento || null, qtd_parcelas || null, dias_vencimento || null,
